...
*К сожелению в CPT нельзя реализовать подобную настройку, в связи с чем web сервер я добавил в виде отдельного сервера в 20 vlan скоммутированного на 24 порт коммутатора S1 и IP адресом 10.20.0.5*
...
S1(config-if)#
```
*Кроме этого, для "симуляции" наличия веб сервера на интерфейсе Loopback сделал следующее: Настройки Loopback интерфейса менять не стал, но включил порт gigabitEthernet 0/0/0 на маршрутизаторе R1, назначил ему адрес 172.16.2.1/24 и подключил к нему веб сервер с адресом 172.16.2.2/24*
```
...
![](./images/lab_11_fig_11.png)

*Компьютер PC-B*

![](./images/lab_11_fig_12.png)

![](./images/lab_11_fig_13.png)

![](./images/lab_11_fig_14.png)

![](./images/lab_11_fig_15.png)
